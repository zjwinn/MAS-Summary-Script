...
```{r inputs}
# VCF file name
vcf_file <- file.path(getwd(), "Example.vcf.gz")

# MAS key
mas_key_file <- file.path(getwd(), "Example_File.txt")
...
vcf[, 10:ncol(vcf)] <- lapply(vcf[, 10:ncol(vcf)], function(x) {
  substr(x, 1, 3)
})
...
vcf_to_hapmap <- function(x, verbose = FALSE) {
  ...
  if (temp2 %in% c("A", "T", "G", "C") & temp3 %in% c("A", "T", "G", "C")) {
    ...
    temp1[, 1:ncol(temp1)] <- lapply(temp1[, 1:ncol(temp1)], function(x) {
      gsub(0, temp2, x)
    })
    temp1[, 1:ncol(temp1)] <- lapply(temp1[, 1:ncol(temp1)], function(x) {
      gsub(1, temp3, x)
...
# If duplicated
if (any(duplicated(key[, "Locus"])) == TRUE) {
  # Throw error message and stop
  stop("There appears to be duplicated loci in the key file. Please check your inputs!")
}

# For loop in parallel
reports <- foreach(i = unique(key[, "Locus"])) %dopar% {
  # Alternate for sequential option
  ...
  # Pull marker call in second collumn
  temp3.1 <- c(temp3[, 2])
  names(temp3.1) <- temp3[, 1]

  # Finish pulling makrers
  temp3 <- temp3[, 12:ncol(temp3)]
  temp3 <- temp3[temp2, ]

  # Check if markers are found in both files
  if (all(temp2 %in% rownames(temp3)) == FALSE) {
    # Send Message for sequential option
    # message("#################################")
    ...
    warning_message <- paste("Warning: Markers identified for ",
      i,
      " are not found HapMap.\n",
      " Not producing call for locus ",
      i,
  ...
  if (is.na(temp1[, "Pos"])) {
    ...
    temp4 <- unlist(strsplit(temp1[, "Pos"], "_"))
  }
  if (is.na(temp1[, "Het"])) {
    temp5 <- NA
  } else {
    temp5 <- unlist(strsplit(temp1[, "Het"], "_"))
  }
  if (is.na(temp1[, "Neg"])) {
    temp6 <- NA
  } else {
    temp6 <- unlist(strsplit(temp1[, "Neg"], "_"))
  }
  if (is.na(temp1[, "Missing"])) {
    temp7 <- NA
  } else {
    temp7 <- unlist(strsplit(temp1[, "Missing"], "_"))
  }

  # set auto_suggestions to null
  auto_suggestions <- c()
  ...
  if (temp1[, "Auto"] == TRUE) {
    ...
    if (is.na(temp4[1])) {
      stop(
        "It appears there are no positive haplotypes given for ",
        i,
    ...
    if (is.na(n.missing) | !is.numeric(n.missing)) {
      stop(
        "Number of tolerated missing calls must be defined and must be a number for ",
        i,
    ...
    if (length(temp2) == 1) {
      stop(
        "Only one defining marker detected for ",
        i,
    ...
    for (j in rownames(temp3)) {
      temp1.1 <- unlist(strsplit(temp3.1[j], "/"))
    ...
    positive <- cbind(positive, matrix(
      data = unlist(strsplit(temp4, split = ":")),
      nrow = nrow(positive),
      byrow = TRUE
    ))
    ...
    if (nrow(positive) == 0) {
      stop(
        "It appears that no positive cases for ",
        i,
        " are complete case!",
    ...
    if (length(unique(is.na(heterozygous))) != 1) {
      ...
      heterozygous <- data.frame(
        Haplotype = paste(heterozygous, collapse = ":"),
        t(heterozygous)
      )
      colnames(heterozygous)[2:ncol(heterozygous)] <- ref_alt[, "Marker"]
    }
    ...
    # Define missing case
    missing <- data.frame(
      Haplotype = paste(rep("N", length(2:ncol(heterozygous))), collapse = ":"),
    ...
    # Combine the cases
    cases <- data.frame(
      Type = c("Positive", "Heterozygous", "Negative", "Missing"),
  ...
  if (is.data.frame(auto_suggestions) == TRUE) {
    # return
    return(list(
      Report = temp12,
      AutoSuggestions = temp1
    ))
  } else {
    # return
    return(temp12)
...
# Figure out where is the first full report
for (i in 1:length(reports)) {
  # Pull report
  temp1 <- reports[[i]]

  # If it is a dataframe, report that index
  if (is.data.frame(temp1) == TRUE) {
    j <- i
  }

  # If j is no longer 0, exit the loop
  if (j != 0) {
    break
  }
}
...
for (i in 1:length(reports)) {
  # Pull report
  temp1 <- names(reports[[i]])

  # If it is a dataframe, report that index
  if (length(temp1) == 2 & all(c("Report", "AutoSuggestions") %in% temp1)) {
    k <- i
  }
...
# Now merge the parallel output
for (i in j:length(reports)) {
  # Pull report
  temp1 <- reports[[i]]

  # If the report is not a dataframe
  if (is.character(temp1) == TRUE) {
    # Show message
    cat("############################\n")
    cat(temp1)
    cat("############################\n")
    ...
    # Go to next thing
    next
  } else if (i == j & i != k & is.data.frame(temp1) == TRUE) {
    marker_report <- temp1
  } else if (i > j & i != k & is.data.frame(temp1) == TRUE) {
    # Merge marker report
    marker_report <- suppressWarnings(merge(marker_report, temp1, by = "Line"))
  } else if (i == j & i == k & is.list(temp1) == TRUE) {
    auto_suggestions <- temp1[[2]]
    marker_report <- temp1[[1]]
  } else if (i > j & i == k & is.list(temp1) == TRUE) {
    auto_suggestions <- temp1[[2]]
    marker_report <- suppressWarnings(merge(marker_report, temp1[[1]], by = "Line"))
  } else if (i > j & i >= k & is.list(temp1) == TRUE) {
    auto_suggestions <- rbind(auto_suggestions, temp1[[2]])
    marker_report <- suppressWarnings(merge(marker_report, temp1[[1]], by = "Line"))
  }
...
for (i in colnames(marker_calls)[2:ncol(marker_calls)]) {
  ...
  temp3 <- data.frame(
    ...
    sum(is.na(temp1)),
    ifelse(nrow(temp2[temp2$temp1 == "UNIDENTIFIED", "N"]) == 0,
      0,
      temp2[temp2$temp1 == "UNIDENTIFIED", "N"]
    ),
    length(temp1)
  )
  colnames(temp3) <- c(
...
for (i in colnames(marker_summary)[2:ncol(marker_summary)]) {
  # Check for partial matches
  if (length(grep(i, key[, "Locus"])) > 1) {
    # Length of the base string 'i'
    len_i <- nchar(i)

    # Length of mismatches
    len_all <- nchar(key[, "Locus"][grep(i, key[, "Locus"])])

    # Pull correct string and all incorrect strings
    exact_i_index <- which(len_all == len_i)

    # Pull correct string
    correct_i <- key[, "Locus"][grep(i, key[, "Locus"])[exact_i_index]]

    # Pull incorrect string
    incorrect_i <- key[, "Locus"][grep(i, key[, "Locus"])[-exact_i_index]]

    # Pull data (subset marker_report)
    temp1 <- as.data.frame(marker_report[, c("Line", colnames(marker_report)[grep(pattern = correct_i, colnames(marker_report))])])

    # Drop the incorrect i
    for (j in 1:length(incorrect_i)) {
      # Intersect with list of genes
      string_toss <- colnames(temp1)[grep(pattern = incorrect_i[j], colnames(temp1))]
      string_toss <- string_toss[!string_toss %in% paste(correct_i, key_markers, sep = "_")]
      temp1 <- temp1[, !colnames(temp1) %in% string_toss]
      remove(string_toss)
    }
